...
class jsonConvert(object):
    ...
    def convert_text_func(self,content) -> str:
        """ 正常文本、粗体、斜体、删除线、链接"""
        all_text = ''
        one_four_content = content.get('4')
        if one_four_content:
            one_four_s = one_four_content.get('s')
            if one_four_s and one_four_s.get('ti') and one_four_s.get('ti') == 28:
                all_text += '\t'
...
class YoudaoNoteConvert(object):
    ...
    def covert_xml_to_markdown_content(file_path):
        ...
        for element in list(body_element):
            ...
            new_content_list.append(f'\r\n' if len(line_content) == 0 else line_content)
